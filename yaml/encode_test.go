...
func TestMarshal(t *testing.T) {
	...
	for _, item := range marshalTests {
		...
		assert.Nil(err)
		assert.Equal(item.data, string(data))
	}
}
...
func TestEncoderSingleDocument(t *testing.T) {
	...
	for _, item := range marshalTests {
		...
		err = enc.Close()
		assert.Nil(err)
		assert.Equal(item.data, buf.String())
	}
}
...
func TestEncoderWriteError(t *testing.T) {
	...
	err := enc.Encode(map[string]string{"a": "b"})
	assert.Equal(`yaml: write error: some write error`, err.Error())
}
...
func TestMarshaler(t *testing.T) {
	...
	for _, item := range marshalerTests {
		...
		assert.Nil(err)
		assert.Equal(string(item.data), string(data))
	}
}
...
func TestSortedOutput(t *testing.T) {
	...
	for i, k := range order {
		...
		if index < last {
			assert.FailNow(fmt.Sprintf("%#v was generated before %#v: %q", k, order[i-1], out))
		}
		last = index
	}
}
