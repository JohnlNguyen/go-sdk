...
func TestUnmarshal(t *testing.T) {
	...
	for _, item := range unmarshalTests {
		...
		assert.Equal(item.value, value.Elem().Interface())
	}
}
...
func TestUnmarshalFullTimestamp(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal(time.Date(2015, 2, 24, 18, 19, 39, 123456789, ts.Location()), ts)
	assert.Equal(time.Date(2015, 2, 24, 21, 19, 39, 123456789, time.UTC), ts.In(time.UTC))
}
...
func TestDecoder(t *testing.T) {
	...
	for _, item := range decoderTests {
		...
		assert.Equal(item.values, values)
	}
}
...
func TestDecoderReadError(t *testing.T) {
	...
	assert.NotNil(err)
	assert.Equal(`yaml: input error: some read error`, err.Error())
}
...
func TestUnmarshalNaN(t *testing.T) {
	...
	assert.Nil(err)
	assert.True(math.IsNaN(value["notanum"].(float64)))
}
...
func TestUnmarshalerPointerField(t *testing.T) {
	...
	for _, item := range unmarshalerTests {
		...
		assert.Nil(err)
		if item.value == nil {
			assert.Nil(obj.Field)
		} else {
			assert.NotNil(obj.Field)
			assert.NotNil(item.value, obj.Field.value)
		}
	}
}
...
func TestUnmarshalerValueField(t *testing.T) {
	...
	for _, item := range unmarshalerTests {
		...
		assert.Nil(err)
		assert.NotNil(obj.Field)
		assert.Equal(item.value, obj.Field.value)
	}
}
...
func TestUnmarshalerWholeDocument(t *testing.T) {
	...
	assert.True(ok)
	assert.Equal(unmarshalerTests[0].value, value["_"])
}
...
func TestUnmarshalerTypeError(t *testing.T) {
	...
	assert.NotNil(v.M["abc"])
	assert.Nil(v.M["def"])
	assert.NotNil(v.M["ghi"])
	assert.Nil(v.M["jkl"])

	assert.Equal(1, v.M["abc"].value)
	assert.Equal(3, v.M["ghi"].value)
}
...
func TestUnmarshalerRetry(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal(sliceUnmarshaler([]int{1, 2, 3}), su)

	err = yaml.Unmarshal([]byte("1"), &su)
	assert.Nil(err)
	assert.Equal(sliceUnmarshaler([]int{1}), su)
}
...
func TestUnmarshalNull(t *testing.T) {
	...
	for _, test := range unmarshalNullTests {
		...
		if reflect.TypeOf(item).Kind() == reflect.Map {
			assert.Equal(reflect.MakeMap(reflect.TypeOf(item)).Interface(), reflect.ValueOf(item).Interface())
		} else {
			assert.Equal(zero, reflect.ValueOf(item).Elem().Interface())
		}
	}
}
